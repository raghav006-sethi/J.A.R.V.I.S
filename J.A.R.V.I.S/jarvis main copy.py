...
def play_song(query):
    ...
    song=query.partition('play')[2].split()
    # Open YouTube page for each search term
    driver.get('https://www.youtube.com/results?search_query={}'.format(song))
    # Find a first webelement with video thumbnail on the page
    link_webelement = driver.find_element(By.CSS_SELECTOR, 'div#contents ytd-item-section-renderer>div#contents a#thumbnail').get_attribute('href')
    # Grab webelement's href, this is your link, and store in a list of links:
    webbrowser.open_new(link_webelement)
...
'''test this'''
def change_volume(query, step=10):
    ...
    step_percent = step / 100.0

    # Adjust volume
    if "increase" in query:
        new_volume = min(current_volume + step_percent, 1.0)
        action="increase"  # Ensure max volume is 1.0 (100%)
    elif "decrease" in query:
        new_volume = max(current_volume - step_percent, 0.0) 
        action="decrease" # Ensure min volume is 0.0 (0%)
    elif "mute" in query:
        new_volume = max(current_volume - current_volume, 0.0) 
        action="decrease" # Ensure min volume is 0.0 (0%)
    else:
    ...
    say(f"Volume {action}d to {new_volume * 100:.0f}%")
...
def Execution(command):
    try:
        ...
        if site_found == 0 and app_found == 0:  # Continue only if neither site nor app was found
            ...
            elif any(word in command for word in ['increase','decrease','mute']):
